...
class MaskRCNN():
    ...
    def compile(self, learning_rate, momentum):
        ...
        for name in loss_names:
            ...
            #TODO if layer.output in self.keras_model.losses:
            equal = tf.math.equal(layer.output, self.keras_model.losses)
            if equal.shape == (len(loss_names),):
                continue
            loss = (
                tf.reduce_mean(layer.output, keepdims=True)
                * self.config.LOSS_WEIGHTS.get(name, 1.))
            self.keras_model.add_loss(loss)
            print(len(self.keras_model.losses))
